...
function _initTransaction(model){
    ...
    return new Promise((resolve,reject)=>{
        ...
        sql.query(query,(err,res)=>{
                ...
                sql.query("INSERT INTO `transaction_master` (`userid`, `amount`, `transtype`, `orderid`, `paymentstatus`,`commissionpercentage`,`commissionamount`,`settlementamount`) SELECT userid,?,'bill',billid,1,?,?,? FROM bill_transaction_master WHERE billid = ?;",[model.amount,global.billPayCommission,parseFloat(convenienceFee),parseFloat(convenienceFee)- model.amount,orderid],(err,res)=>{
                    if(err){
