...
function _getManPower(areaid){
    return new Promise((resolve,reject)=>{
        sql.query("SELECT A.manreqid,A.bid,A.title,A.jobtype,A.description,A.skill,A.qualification,A.experience,A.agefrom,A.ageto,A.salaryfrom,A.salaryto,A.gender,A.employementtype,A.noofvacancies,A.startdate,A.enddate,C.name as businessName,C.profile as businessProfile, D.phone as businessPhone FROM manpower_req_master as A ,location_master as B , business_info as C, contact_info as D,business_master as E WHERE A.bid = B.uid AND A.bid = C.uid AND A.bid = D.uid AND B.areaid = ? AND E.bid = A.bid AND E.active = 1 order by A.manreqid DESC;",[areaid],(err,data)=>{
                if(err){
                    reject();
