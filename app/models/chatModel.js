...
function _getUnreadChats(roomid,chatid){
    return new Promise((resolve,reject)=>{
        sql.query("SELECT chatid,roomid,senderid,sendertype,message,media,mediatype,isseen,IF(DATE(NOW()) = DATE(date),DATE_FORMAT(date, '%l:%i %p'),DATE_FORMAT(date, '%D %b %Y %l:%i %p')) as date FROM chat_master WHERE roomid = ? AND chatid > ? ORDER BY chatid ASC;",[roomid,chatid],(err,data)=>{
                if(err){
                    reject(err);
                    console.log('Chats Fetch Failed due to '+err);
                    return;
